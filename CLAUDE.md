...
- **Server Actions** pour toutes les mutations (pas d'API REST), sauf 4 routes : auth NextAuth, webhook Stripe, flux SSE, sonde `/api/health` (déploiement)
...
- **Déploiement** : VPS OVH, systemd (`triage-app@3002` / `triage-app@3003`, bascule bleu/vert) + Nginx (même instance que of-qualiopi) — PM2 abandonné le 09/08/2026, cf. § Déploiement
...
src/
  ...
  app/api/
    ...
    health              # sonde du déploiement bleu/vert (200, sans accès base)
...
- **VPS OVH** `debian@145.239.55.58` (box `semawe-prod-gra11`, même instance que of-qualiopi), `ssh -i ~/.ssh/id_semawe_vps2`. App dans `/home/debian/triage-app`, base PostgreSQL locale `triageapp_prod`, service **systemd** — **plus PM2** depuis le durcissement du 09/08/2026 : `pm2 list` sous `debian` ne renvoie rien, ce n'est pas un incident. Depuis le 15/08/2026 le service mono-instance `triage-app.service` est remplacé par le **template `triage-app@.service`, deux instances nommées par leur port : `triage-app@3002` et `triage-app@3003`** (les deux couleurs du déploiement bleu/vert, **bindées 127.0.0.1**, chacune servant son propre répertoire de build `.next-<port>`). Une seule est démarrée en régime normal ; `systemctl status triage-app` sans instance ne renvoie plus rien d'utile. Drop-in `/etc/systemd/system/triage-app@.service.d/durcissement-a5.conf` (action A5 de l'audit du 03/08) : `MemoryMax=768M`, `CPUQuota=80%`, `NoNewPrivileges`, `PrivateTmp`, `ProtectSystem=full`, familles d'adresses restreintes ; `ProtectHome` en est volontairement absent, le code vivant sous `/home/debian`. Nginx → triapp.fr, SSL Let's Encrypt. (Ancienne box `51.178.234.59` compromise et supprimée le 28/06/2026 — incident SEC-21 ; reconstruction propre + durcissement : egress UFW deny outgoing, vps-watch, fail2ban.)
- **Auto-déploiement par webhook, sans coupure** : tout push sur `main` déclenche `webhook-server.js` (sous l'identité `deploybot`) → **`/opt/deploybot/deploy-triage-app.sh`**. Le script lit la couleur en service dans `/etc/nginx/conf.d/triapp-upstream.conf`, puis, sur la **couleur inactive** : `git pull origin main` → `npm ci --ignore-scripts` → `prisma migrate deploy` → `NEXT_DIST_DIR=.next-<port cible> npm run build`. **L'instance en service n'est pas touchée par ces étapes** : un build qui échoue laisse la production intacte, sans intervention. La nouvelle instance est ensuite démarrée, interrogée sur `GET /api/health` jusqu'à 180 s, et ce n'est qu'une fois qu'elle répond que nginx pivote dessus (`systemctl reload`, gracieux) ; l'ancienne est arrêtée après 20 s de drain. **Pas besoin de SSH pour déployer**, mais c'est `migrate deploy` (pas `dev`) qui tourne en prod.
- **Gestes privilégiés du déploiement** : `deploybot` n'a jamais reçu le droit d'écrire dans `/etc/nginx` ni de piloter systemd — le durcissement A3 du 09/08 l'a réduit à « devenir les trois comptes applicatifs, redémarrer les trois services ». La bascule passe donc par **`/usr/local/sbin/triapp-bascule`** (root:root 0755, verbes `demarrer|activer|arreter` + port validé), ouvert par six entrées sudo littérales dans `/etc/sudoers.d/deploybot`. Ne pas contourner cette voie en élargissant les droits de `deploybot`.
- **Miroirs versionnés** : `deploy/` contient les copies de référence de `deploy-triage-app.sh`, `triapp-bascule`, `triage-app@.service`, des deux fichiers nginx et de la règle sudoers. **Seule la copie du serveur s'exécute** : le miroir du script avait dérivé entre le 25/06 et le 10/08/2026 sans que la prod bronche — le réaligner à chaque modification, dans les deux sens. Le `mv .next .next.old` d'avant le 15/08 n'existe plus (il retirait les chunks sous les pieds du processus encore vivant) ; `.next` et `.next.old` ont été supprimés, chaque couleur ayant son `.next-<port>`.
...
- **Vérif post-deploy** — dans cet ordre, `https://triapp.fr/fr` → 200 ne prouve rien à lui seul (l'ancien build répond aussi) :
  ...
  2. `ssh … 'tail -5 /var/log/deploybot/deploy-triage-app.log'` se termine par `=== Deploy OK (en service sur <port>) ===` (sinon le déploiement est en cours ou a échoué) ;
  3. `ssh … 'grep -o "127.0.0.1:[0-9]*" /etc/nginx/conf.d/triapp-upstream.conf'` — la couleur a changé (**pas** `pm2 list`, vide depuis le passage à systemd) ;
  ...
  **Un 502 pendant un déploiement n'est plus normal** : jusqu'au 15/08/2026, `systemctl restart` sur l'instance unique servait des 502 le temps que `next start` réécoute (tâche #1652). La fenêtre est variable, et c'est ce qui rendait le défaut trompeur : mesurée à ~1 s sur une box sans trafic (le 15/08, `Ready in 393ms`), elle atteignait la minute ou deux quand des flux SSE ouverts retenaient le `SIGTERM` jusqu'au `TimeoutStopSec` — donc précisément quand une réunion était en cours. Avec la bascule bleu/vert, nginx ne pointe jamais vers un port qui n'écoute pas. Un 502 est désormais un incident à instruire : lire `/var/log/deploybot/deploy-triage-app.log` et `systemctl status triage-app@3002 triage-app@3003`.

  **Contrainte SSE pendant la bascule** : le broker `src/lib/sse.ts` est in-process, donc les quelques secondes où les deux couleurs coexistent, un client encore attaché à l'ancienne ne reçoit pas les événements émis par la nouvelle. Les `EventSource` se reconnectent seuls sur la couleur active dès l'arrêt de l'ancienne. Une réunion en cours au moment d'un déploiement perd donc au pire quelques secondes de temps réel, là où elle perdait toute la fenêtre de coupure avant le 15/08.
