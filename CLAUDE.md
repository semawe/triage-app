...
- **Vérif post-deploy** — dans cet ordre, `https://triapp.fr/fr` → 200 ne prouve rien à lui seul (l'ancien build répond aussi) :
  ...
  **Un 502 pendant un déploiement n'est plus normal** : jusqu'au 15/08/2026, `systemctl restart` sur l'instance unique coupait le service une à deux minutes à chaque déploiement (tâche #1652). Avec la bascule bleu/vert, nginx ne pointe jamais vers un port qui n'écoute pas — mesuré sur un déploiement réel, 0 réponse non-200 sur une boucle de `curl` toutes les 0,5 s. Un 502 est donc désormais un incident à instruire : lire `/var/log/deploybot/deploy-triage-app.log` et `systemctl status triage-app@3002 triage-app@3003`.
